...
public class SolutionPdfExportService {
    ...
    public byte[] generateSolutionPdf(Solution solution) {
        ...
        try {
            ...
            exerciseBody.addElement(
                    row("Exercise title: ", safe(exercise != null ? exercise.getTitle() : null), labelFont, bodyFont));
            exerciseBody.addElement(row("Exercise description: ",
                    safe(exercise != null ? exercise.getDescription() : null), labelFont, bodyFont));
            ...
            solutionBody.addElement(row("Author: ",
                    safe(solution.getOwner() != null ? solution.getOwner().getName() : null), labelFont, bodyFont));
            solutionBody.addElement(row("Exported at: ", EXPORT_DATE_FORMAT.format(java.time.LocalDateTime.now()),
                    labelFont, bodyFont));
            solutionBody.addElement(new Paragraph(" "));
            solutionBody
                    .addElement(row("Solution Description: ", safe(solution.getDescription()), labelFont, bodyFont));
    ...
    private static class DsgramPageEvent extends PdfPageEventHelper {
        ...
        public void onEndPage(PdfWriter writer, Document document) {
            ...
            ColumnText.showTextAligned(over, Element.ALIGN_LEFT,
                    new Phrase("Corporate solution export", headerSubtitleFont),
    ...
    public byte[] sendCreationPdf(SolutionPDFInfoDTO solPdf, ExerciseBasicInfoDTO exerciseDTO,
            Blob pdfBlob) {
        SolutionPdfExportRequestDTO requestBody = new SolutionPdfExportRequestDTO(
                solPdf,
                exerciseDTO,
                toBytes(pdfBlob));
    ...
    private byte[] toBytes(Blob blob) {
        ...
        try {
            ...
            throw new RuntimeException("Unable to read solution PDF blob", e);
        }
    }
}
