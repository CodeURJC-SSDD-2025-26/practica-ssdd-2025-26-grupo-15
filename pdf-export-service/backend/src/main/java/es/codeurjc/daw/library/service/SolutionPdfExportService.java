...
public class SolutionPdfExportService {
    ...
    public byte[] generateSolutionPdf(SolutionPDFInfoDTO solution,ExerciseBasicInfoDTO exercise, byte[] solImage) {
        if (solution == null) {
        ...
        try {
            ...
            exerciseBody.addElement(row("Exercise title: ", safe(exercise != null ? exercise.title() : null), labelFont, bodyFont));
            exerciseBody.addElement(row("Exercise description: ", safe(exercise != null ? exercise.description() : null), labelFont, bodyFont));
            ...
            solutionBody.addElement(row("Solution name: ", safe(solution.name()), labelFont, bodyFont));
            solutionBody.addElement(row("Author: ", safe(solution.owner() != null ? solution.owner().name() : null), labelFont, bodyFont));
            ...
            solutionBody.addElement(row("Solution Description: ", safe(solution.description()),labelFont, bodyFont));
            solutionDetail.addCell(solutionBody);
            document.add(solutionDetail);

            byte[] imageBytes = solImage != null ? solImage : extractImageBytes(null);
    ...
    private byte[] extractImageBytes(Blob imageBlob) {
        if (imageBlob == null) {
